      ...
      <nav
        className={`fixed top-0 left-0 right-0 z-[60] transition-all duration-500 ${
          isOpen
            ? "bg-transparent py-5"
            : scrolled
              ? "bg-dark/95 backdrop-blur-sm py-3"
              : "bg-transparent py-5"
        }`}
      >
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
          ...
          <a href="#" className="absolute left-1/2 -translate-x-1/2">
            <span
              className={`font-display italic text-cream transition-all duration-500 drop-shadow-lg ${scrolled && !isOpen ? "text-xl" : "text-2xl"}`}
            >
              U Jurčíka
            </span>
          </a>
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="hidden md:inline-flex items-center gap-2 px-4 py-2 bg-gold/90 hover:bg-gold text-dark text-xs font-body uppercase tracking-[0.15em] transition-all"
          >
